...
rel.spdsht$addresses <-   str_replace_all(rel.spdsht$addresses, "United Kingdom|UK", "United_Kingdom") 
...
Green = pre-print/publisher's version can be archived   
```{r updateOA}
...
#### Analysis: Where are most authors coming from?  
...
#### Analysis: Authorship  
*How many different first author countries?*  
```{r}
...
### How many include authors from the region?  
...
In the last ten years, 49.74% of papers have included someone from within country, 42.54% of which were led by the first author.   
```{r}
...
### Type of research  
### Usability of literature over time   



```{r}
author.countries.df %>% group_by(year,`Type of Paper`) %>% 
  summarize(n=n()) %>% 
