...
year.countries.df <- author.countries.df %>% 
  separate_wider_delim(.,cols=countries_all,delim=",",names_sep="_",too_few="align_start") %>%
  select(c(year,starts_with("Type"),language,starts_with("countr"))) %>% 
  #group_by(year) %>% summarize(across(starts_with("count"),n_distinct))
pivot_longer(cols=c(starts_with("countries")),
              #names_to="year",
              values_drop_na=TRUE
              ) %>% group_by(year,value) %>% 
  summarize(countries_peryear = n()) %>%
  ungroup() %>% mutate(value=trimws(value,which="left")) %>%
  #group_by(value) %>%  
  add_count(value,sort=TRUE) %>% #filter(n>1) %>%
  mutate(value=factor(value,levels=rev(unique(value)),ordered=TRUE)) 
```
...
```{r}
year.countries.df %>% ggplot(aes(x=year,y=value)) + 
  geom_point(aes(size=countries_peryear))
```

```{r}
year.countries.df <- 
  author.countries.df %>% 
  separate_wider_delim(.,cols=countries_all,
                       delim=",",names_sep="_",
                       too_few="align_start") %>%
  select(c(year,starts_with("Type"),language,starts_with("countr"))) %>% 
  #group_by(year) %>% summarize(across(starts_with("count"),n_distinct))
pivot_longer(cols=c(starts_with("countries")),
  ...
  mutate(firstauth=case_when(name=="countries_all_1" ~ n(),
                             .default=NA)) %>%
  ungroup() %>%
  group_by(year,value) %>% 
  reframe(countries_peryear = n(),
            firstauth=unique(firstauth)) %>% 
  group_by(year,value) %>% 
  filter(n()>1 & !is.na(firstauth) | n()==1) %>% ungroup() %>%
  add_count(value,sort=TRUE) %>% 
  mutate(value=factor(value,levels=rev(unique(value)),ordered=TRUE)) 
```
```{r}
year.countries.df %>% ggplot(aes(x=year,y=value,color=firstauth)) + 
  geom_point(aes(size=countries_peryear),alpha=0.8) + scale_color_gradient(na.value="grey") + theme_bw() + theme(text=element_text(size=14)) + scale_size_continuous(range=c(3,10))

ggsave("/Users/lauragivens/Downloads/roughdraft2.png",width=14,height=12)
```
...
pivot_longer(cols=c(starts_with("countries")),
  ...
  reframe(countries_peryear = n(),
            firstauth=unique(firstauth)) %>% 
  group_by(year,value) %>% 
  filter(n()>1 & !is.na(firstauth) | n()==1) %>% ungroup() %>% group_by(value) %>% 
  mutate(total_percountry = sum(countries_peryear,na.rm=TRUE),
         total_firstauth = sum(firstauth,na.rm=TRUE)) %>% 
  add_count(value,sort=TRUE) %>% 
  mutate(value=factor(value,levels=rev(unique(value)),ordered=TRUE)) 
```
...
```{r}
year.countries.df %>% group_by(value) %>% 
  reframe(total_percountry = sum(countries_peryear,na.rm=TRUE),
         total_firstauth = sum(firstauth,na.rm=TRUE)) %>% 
ggplot() + 
  geom_point(aes(x=total_percountry,y=value)) + 
  geom_point(aes(x=ifelse(total_firstauth>0 & !duplicated(value),total_firstauth,NA),y=value),alpha=0.5,color="red")
```
