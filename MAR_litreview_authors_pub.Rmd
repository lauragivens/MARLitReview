...
eco.table <- geo.df %>% group_by(value,Focus) %>% 
  ...
  flextable::theme_zebra() %>%
  #flextable::set_table_properties(layout="autofit") %>% 
  flextable::fontsize(size=20,part="all") %>% hline(i=14) %>% vline(j=6) %>%
...
names(labs.hm) <- c("1991-1995", "1996-2000","2001-2005","2006-2010","2011-2015","2016-2020","2021-2025","<1991")
...
comparison.df %>% 
  ...
  theme_void() + theme(text=element_text(size=18)) + guides(fill=guide_legend(direction="vertical",position="right"))
...
```
```{r marfocushm}  
MARcountries_heatmap <-  geo.df.regionFocus %>%
  mutate(yearBin=case_when((year>1990 & year<1996) ~ 1,
                           (year>=1996 & year<2001) ~ 2,
                           (year>=2001 & year<2006) ~ 3,
                           (year>=2006 & year<2011) ~ 4,
...
ggsave("Figures/MARcountries_heatmap.png",width=15,height=7)
```
...
country.ecosystem.plot2 + 
  ...
  guides(fill=guide_colorbar(position="top",
                             title="Number of papers")) + theme(text=element_text(size=20))
