...
eco.df %>% 
  ...
  mutate(across(everything(),~case_when(str_detect(.,"Unsure") ~ NA,
                         str_detect(.,"No") ~ NA,
                         str_detect(.,"N/A") ~ NA,
                         ...
                         .default=.))) %>% 
  mutate(across(everything(), ~ case_when(str_detect(.,"Yes") ~ 1, .default=0))) %>% 
  colSums()
```
...
```{r}
participation.df %>% filter(Oldyear>2014 & Oldyear <2025) %>%
  mutate(n=Lead/sum(Lead,Coauthor,na.rm=TRUE)) %>% 
  .$n %>% mean()
```
...
#ggsave("Figures/t15_AuthorsByYear_dotplot.png",subs.country.authors.dotplot,      width=10,height=6,bg="white")
```
...
MARmp_hm_df_perc <- 
  ...
  pivot_wider(names_from = c(Country), #output columns
            names_sep="_",
            names_sort=TRUE,
...
wrap_plots(MARcountries_heatmap_10yr,
           plot_spacer(),
           gen_grob(MARmp_perc, fit = "width", #fit = "fixed",
                    wrapping=FALSE,just = "center"),
           #gen_grob(MARmp_perc_long, fit = "width"),
           nrow=3) + plot_layout(heights=c(5,1,7))

ggsave("Figures/MARcountries_mp_table_perc.png",width=10,height=10)
```

# 7. Tables  
```{r t7b}
write.csv(MARmp_hm_df_perc,"Figures/7b.RegionalNuanceinManagementPlans.csv")
```
...
geo.df.regionFocus %>% 
  group_by(Topic_all,GeographicFocus) %>%
    summarize(n=sum(Topic_all_num)) %>% 
    pivot_wider(names_from=Topic_all,values_from=n,values_fill=0) %>% 
    #filter(value%in%top15) %>%
    column_to_rownames(.,"GeographicFocus") %>% 
   # mutate(Total=rowSums(.,na.rm=TRUE)) %>% #filter(Total>1) %>%
    rownames_to_column("Country") %>%
    #arrange(desc(Total)) %>% 
   # bind_rows(summarise(.,across(where(is.numeric),sum),
    #                    across(where(is.character),~"Total"))) 
write.csv(.,"Figures/ST1.TopicalFocusbyGeography.csv")
```
...
geo.df.countryFocus %>% 
  group_by(Topic_all,Country) %>%
   ...
   # bind_rows(summarise(.,across(where(is.numeric),sum),
    #                    across(where(is.character),~"Total"))) 
write.csv(.,"Figures/ST2.PublishingTopicbyCountry.csv")
```
...
#43 countries  
```
How many countries were involved prior to 2006?  
```{r}
year.countries.df %>% filter(year < 2006) %>% .$value %>% unique() %>% length()
```
Most productive countries overall? (including first and coauthors)  
```{r}
top15
#Mexico     USA        UK         Australia  Germany    Canada     Belize     Spain    Honduras   Panama     Guatemala  Austria    Costa_Rica Chile      Norway
```
Number of papers published by the most productive countries?  
```{r}
#number of papers published by countries in the top 15 most productive category
year.countries.df %>% filter(value %in%top15) %>% 
  select(c(value,total_percountry,total_firstauth)) %>% 
...
year.countries.df %>% filter(!value %in%top15) %>% 
  ...
  filter(!is.na(total_percountry) & !is.na(total_firstauth)) %>% .$total_firstauth %>% sum()
```
(total percentage of papers published by productive countries):  
289/(289+18)  

In the last decade, 48.55% of papers have included someone from within country. However, 42.8% of the papers were led by a first author primarily affiliated with an instutition within country (87.34% of the papers that included an author from within country)   
```{r}
participation.df %>% filter(Oldyear>2014 & Oldyear <2025) %>% 
  .$perc %>% mean(.,na.rm=TRUE)
participation.df %>% filter(Oldyear>2014 & Oldyear <2025) %>% 
