...
mutate(nval=n())  %>% summarize_at(vars(-group_cols()),~sum(!is.na(.))) %>%
  ...
  mutate("Topic_new"=case_when(str_detect(Topic_all,"MP") ~ "Policy",
                        ...
                        str_detect(Topic_all,"ender") ~ "Community-based",
                         ...
                         str_detect(Topic_all,"voluntourism") ~ "Community-based",
                         str_detect(Topic_all,"liveli") ~ "Community-based",
                         str_detect(Topic_all,"takeholder") ~ "Community-based",
                        
                        str_detect(Topic_all,"Resilience") ~ "Resilience",
                        
                             .default=Topic_all)) %>%
  mutate(Topic_new=factor(Topic_new,
                          levels=c("Fish and Fisheries",
                                   "Coral Reefs","Other Ecosystems",
                                   "Mangroves",
                                   "Seagrass",
                                   "Geophysical",
                                   "Policy",
                                   ...
                                   "Resilience",
                                   "nval"))) %>% 
  mutate(Topic_categories = case_when(Topic_new=="Resilience" ~ "Management type",
                                      Topic_new=="Adaptive Management" ~ "Management type",
                                      Topic_new=="Seascape" ~ "Management type",
                                      Topic_new=="Ridge to Reef" ~ "Management type",
                                      Topic_new=="nval" ~ "Total per Region",
...
eco.table <- geo.df %>% group_by(value,Focus) %>% 
  ...
  flextable::flextable(.) %>% 
  flextable::set_header_labels("Geophysical"="Geo.") %>%
  flextable::theme_zebra() %>%
  #flextable::set_table_properties(layout="autofit") %>% 
...
```{r}
geo.df2 %>%
  filter(Topic_categories != "Management type" & Topic_categories != "Total per Region") %>%
  ...
  theme_void() + theme(text=element_text(size=18))
...
geo.df2 %>%
  ...
  group_by(GeographicFocus,Topic_new) %>% 
 mutate(Topic_new_num=sum(Topic_all_num)) %>%
  filter(GeographicFocus=="Belize" | GeographicFocus == "Honduras" | GeographicFocus=="Guatemala" | GeographicFocus=="Mexico") %>%
  
  #group_by(GeographicFocus) %>% 
 # mutate(nval=case_when(Topic_categories=="Total per Region" ~ Topic_new_num,
  #                      .default=1)) %>% 
  
 # mutate(frac=Topic_new_num/nval) %>%
  filter(Topic_new!="nval") %>%
  ggplot(aes(group=Topic_new,
             x=Topic_new,y=Topic_new_num,
             #x="",y=frac,
             fill=Topic_new)) + 
  geom_bar(stat="identity",width=1,color="black") + 
  facet_wrap(~GeographicFocus) + 
  scale_fill_brewer(name="Number of papers",palette="Set3") + 
  theme_bw() + theme(axis.text.x=element_text(angle=45,hjust=1))
```

```{r}
marmp %>% select(c(Country,starts_with("Mention"))) %>% 
  group_by(Country) %>% 
mutate(nval=n())  %>% 
  ...
  filter(name!="Threat" & name!="CharismaticSpp" & name != "ScientistEngagment") %>%
...
comparison.df %>% 
  ...
  facet_wrap(~GeographicFocus) + 
  scale_fill_brewer(name="Proportion of papers",palette="Set3") + 
  theme_void() + theme(text=element_text(size=18)) + guides(fill=guide_legend(direction="horizontal",position="top"))
